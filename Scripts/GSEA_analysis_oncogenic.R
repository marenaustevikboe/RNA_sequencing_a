...
#I compare my filtered data to downloaded C6 (oncogenic) gene sets
...
readr::write_tsv(
  ...
  file.path(
    "Results",
    "SRP123625_gsea_C6_results.tsv"
  )
)

# Use the filter function to select oncogenic terms from gsea results containing "PTEN" in the "ID" column
PTEN_regulated_C6_gene_sets <- gsea_results %>% filter(str_detect(ID, "PTEN"))
print(PTEN_regulated_C6_gene_sets)
glimpse(PTEN_regulated_C6_gene_sets)

#Making dotplot of oncogenic gene sets affected by PTEN deregulation
require(DOSE)
dotplot <- dotplot(PTEN_regulated_C6_gene_sets, showCategory=10, split=".sign", title = "Deregulated pathways affected by PTEN") + facet_grid(.~.sign)
dotplot
...
dotplot <- dotplot + scale_y_discrete(labels = c("Genes down-regulated upon knockdown of PTEN","Genes up-regulated upon knockdown of PTEN"))
dotplot

dotplot <- dotplot + theme(plot.title = element_text(size = 8))

ggplot2::ggsave(file.path("Plots", "SRP123625_gsea_C6_dotplot_PTEN_regulated.png"),
                plot = dotplot
)
