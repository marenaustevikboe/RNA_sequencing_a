...
#Define gene set
mm_curated_sets <- msigdbr(
  species = "Homo sapiens", 
  category = "C2")
head(mm_curated_sets)
...
readr::write_tsv(
  ...
  file.path(
    "Results",
    "SRP123625_gsea_curated_results.tsv"
  )
)


# Use the filter function to select GO terms from gsea results containing "PIP5" in the "core_enrichment" column
PIP5_regulated_geneSets <- gsea_results %>% filter(str_detect(core_enrichment, "PIP5"))
print(PIP5_regulated_geneSets)

#Making dotplot of go-terms affected by PIK3 deregulation
require(DOSE)
dotplot <- dotplot(PIP5_regulated_geneSets, showCategory=10, split=".sign", title = "Stem-cell-assosiated gene sets affected by PIP5K1B") + facet_grid(.~.sign)
dotplot
...
ggplot2::ggsave(file.path("Plots", "SRP123625_gsea_curated_dotplot_PIP5K1B_regulated_stemCellGeneSets.png"),
                plot = dotplot)
